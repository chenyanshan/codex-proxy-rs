...
API Key 持有者可在同一登录页切换登录身份，进入 `/key-usage` 查看自己的用量、按模型统计的 Token 与估算费用、趋势、请求日志、额度与健康时间线；不能访问管理员页面。
...
2. 按需建立账号分组，再创建客户端密钥并选择可用分组，**不选分组表示可使用全部普通账号，不包含 car 独占账号**
3. 打开密钥的「使用密钥」，复制客户端配置

需要多人共享一个账号时，在「分组」中将恰好包含一个独占账号的分组转为 car，再创建代表各使用者的 seat。
账号须有正数有效并发上限；转换后普通 Key 不能再访问该账号，请将已有或新建 Key 加入相应 seat。
同 seat 的 Key 共享手动日／周额度与并发，各自保留 RPM、客户端身份和请求明细；自助页可查看同 seat 各 Key 的费用汇总。
已有 Key 迁入会承接当前窗口的费用，不能退出或跨 seat 转移；费用在请求结束后结算，在途请求可能使最终金额超过限额。
