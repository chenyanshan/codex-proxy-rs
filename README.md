...
打开服务器地址的 `/key-usage`（本机示例：`http://127.0.0.1:8080/key-usage`），或从登录页点击「查询 Key 用量」。无需管理员登录，粘贴客户端 Key 即可查询并手动刷新美元日／七天已记录费用、剩余额度和配置的并发／RPM 上限。当前浏览器记住上次查询成功的 Key；再次打开、刷新或返回页面时，Key 默认遮罩显示并自动查询最新用量。首次无缓存时等待输入，清空可移除记住的 Key。

日额度按北京时间零点重置；七天窗口从首次使用当日零点起算七天，并非自然周。额度为零表示不限，尚未开始的窗口显示「下次使用时确定」。预算已超限仍可查询；无效或停用的 Key 无法查询。接口合同见 [API 参考](docs/api.md)。
