...
`0022_car_seats.sql` 为当前迁移序列追加 car-seat 基础共享能力，支持从当前数据库升级和空库初始化，
不自动把普通分组或 Key 转成 car／seat，也不重置已有预算或费用。配置 car 时需要正数有效账号并发容量。
此序列不能直接接管包含其他同编号迁移的二开数据库；部署前核对迁移历史并备份，不能修改历史 checksum 强行切换。
...
`0023_car_quota_cycles.sql` 在基础共享迁移之后增加账号周期、权重与自动容量发布。已有 seat 权重为 1，
总权重回填为至少seat数量（含停用项、空car至少1）；未确认有效周期前保留手动预算与已用费用。
首次激活按总权重乘130美元初始化周期容量并按权重接管周限额，该数是策略默认值，不是实测或官方容量。
关闭自动发布不关闭周期激活。此上游迁移链不支持接管已部署的个人fork数据库。
