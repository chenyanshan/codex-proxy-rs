...
impl DefaultAccountsService {
    ...
    async fn provider_for_account(
        ...
        (
            AccountPageItem,
            crate::ports::provider_extensions::ProviderAdminHandle,
        ),
        AdminError,
    > {
...
impl AccountsService for DefaultAccountsService {
    async fn list(&self, query: AccountListQuery) -> Result<AccountDirectoryPage, AdminError> {
        let providers = self
            .providers
            .freeze()
            .map_err(|error| map_provider_error(error, "provider directory"))?;
        let runtime = self
            .account_runtime
            .active_rate_limits()
            .await
    ...
    async fn export(
        ...
        let providers = self
            .providers
            .freeze()
            .map_err(|error| map_provider_error(error, "provider export"))?;
        ...
        // 混选时先验证同一发布代次的全部能力，拒绝路径不读取任何一组明文凭据。
        let export_groups = grouped
            ...
            .map(|(provider_kind, ids)| {
                ...
                if !provider.credential_capabilities().export {
                    return Err(AdminError::invalid("所选账号包含不支持导出的平台"));
                }
