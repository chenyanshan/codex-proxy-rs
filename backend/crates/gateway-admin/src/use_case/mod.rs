...
async fn publish_committed_deferred(
    snapshot: &dyn SnapshotControl,
    revision: crate::model::Revision,
) -> Result<(), AdminError> {
    let revision = ConfigRevision::new(revision.get())
        .map_err(|_| AdminError::internal("已提交的配置版本不合法"))?;
    snapshot.publish_committed_deferred(revision).await;
    Ok(())
}

async fn publish_credentials_and_observe_quota(
    provider: &crate::ports::provider_extensions::ProviderAdminHandle,
    snapshot: &dyn SnapshotControl,
    revision: crate::model::Revision,
...
async fn validate_authorization_commit(
    ...
    let matches_target = match (prepared.pending.target(), &prepared.credential) {
        (
            AuthorizationMutationTarget::Create { .. },
            PreparedAuthorizationCredential::Create(credentials),
        ) => {
            ...
            !credentials.is_empty()
                ...
                && credentials.iter().all(|credential| {
                    credential.provider_kind == *provider_kind
                        && identities.insert(&credential.account_id)
                })
        }
        (
