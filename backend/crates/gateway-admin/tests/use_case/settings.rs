...
#[tokio::test]
async fn pricing_accepts_registered_plugin_provider_and_rejects_unknown_sync_provider() {
    let store = Arc::new(PricingSettingsStore::default());
    let services = super::AdminHarness::new()
        .settings(store.clone())
        .provider(super::test_provider("example-plugin"))
        .build()
        .await;
    let context = MutationContext {
        actor: gateway_admin::model::MutationActor::System,
        request_id: "request-plugin-pricing".to_owned(),
    };
    ...
    services
        ...
        .await
        .expect("registered plugin provider pricing");
    assert_eq!(
