...
impl RuntimeSnapshotCompiler {
    ...
    ) -> Result<BTreeMap<ProviderKind, ProviderCatalogGeneration>, RuntimeSnapshotCompileError>
    {
        Ok(self
            .catalogs_for_extensions(extensions)?
            .catalog_generations())
    }

    fn catalogs_for_extensions(
        &self,
        extensions: Option<&crate::runtime::extensions::ExtensionSetReference>,
    ) -> Result<Arc<dyn ProviderCatalogPort>, RuntimeSnapshotCompileError> {
        self.catalogs
            .for_extensions(extensions)
            .map(|catalogs| catalogs.unwrap_or_else(|| self.catalogs.clone()))
