...
async fn key_profile_override_roundtrips_and_explicit_clear_restores_inheritance() {
    ...
    let dynamic = gateway_core::routing::ProviderKind::new("provider.example").unwrap();
    let (_, record) = store
        .create_client_key(
            NewClientKey {
                seat_id: None,
    ...
    assert_eq!(
        record.request_profile_overrides.get(&dynamic),
        Some(&profile)
    );
    ...
    assert_eq!(
        unchanged.request_profile_overrides.get(&dynamic),
        Some(&profile)
    );
    let snapshot = PgRuntimeSnapshotRepository::new(database.pool.clone())
        .load_runtime_snapshot()
    ...
    assert_eq!(
        snapshot.client_api_keys[0].request_profiles.get(&dynamic),
        Some(&profile)
    );
    let mut clear_dynamic = update(None);
    clear_dynamic
        .request_profile_override_updates
        .insert(dynamic.clone(), None);
    let (_, cleared_dynamic) = store
        .update_client_key(clear_dynamic, &context)
        .await
        .unwrap();
    assert!(
        !cleared_dynamic
    ...
    assert_eq!(
        cleared_dynamic.request_profile_overrides.get(&openai),
        Some(&profile)
    );
    let (_, cleared) = store
