...
async fn concurrent_authorization_retries_share_one_account_write_and_one_receipt() {
    ...
    let (authorization, context) = command(vec![credential("acct_receipt", "receipt-user")]);
    ...
    assert_eq!(
        first.result.accounts[0].credential_revision.unwrap().get(),
        1
    );
    ...
    let (mut foreign, _) = command(vec![credential("acct_foreign", "foreign-user")]);
...
async fn a_receipt_failure_rolls_back_accounts_audit_and_config_revision() {
    ...
    sqlx::query("alter table authorization_receipts add constraint fail_receipt check (false)")
        ...
        .await
        .unwrap();
    let (command, context) = command(vec![credential("acct_rollback", "rollback-user")]);
    assert!(
        store
...
async fn expired_receipts_are_unreadable_and_reclaimed_without_removing_accounts() {
    ...
    let store = admin_account_store(&database.pool);
    let (authorization, context) = command(vec![credential("acct_expired", "expired-user")]);
    let key = authorization.key.clone();
    store
    ...
    let (next, context) = command(vec![credential("acct_next", "next-user")]);
...
async fn reauthorization_replay_keeps_the_committed_revision_and_later_profile_edits() {
    ...
    let store = admin_account_store(&database.pool);
    let (seed, context) = command(vec![credential("acct_reauthorize", "reauthorize-user")]);
    let receipt = store.commit_authorization(seed, &context).await.unwrap();
    let id = receipt.result.accounts[0].account_id.clone();
