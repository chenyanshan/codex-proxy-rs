...
use gateway_plugin_runtime::{
    ContinuationDrainConfig, PackageInspector, PackageLimits, PluginRuntime, PluginRuntimeConfig,
    RpcLimits,
};
...
pub struct ModelRequestLedgerFact {
...
impl Environment {
    ...
    pub async fn set_request_profile(
        &self,
        provider: &str,
        ...
        let revision = sqlx::query_scalar::<_, i64>(sqlx::AssertSqlSafe(format!(
            "update {}.runtime_settings
             set provider_request_profiles_json = jsonb_set(
                   provider_request_profiles_json, array[$1], $2::jsonb
                 ),
    ...
    pub async fn provider(
        ...
        self.install_provider(configuration, grants).await;
        self.runtime().await
    }
    ...
    pub async fn install_provider(
        ...
        let permissions = grants
            ...
            .collect();
        let mut contributes = Contributions::from([
            super::contribution_for_id(
                &plugin_id,
                Capability::Executor,
                vec![Stage::Execution],
                configuration
                ...
                configuration
                    .get("output_formats")
                    .map(|value| serde_json::from_value(value.clone()).unwrap())
                    .unwrap_or_else(|| vec!["canonical".into()]),
            ),
            super::contribution_for_id(
                &plugin_id,
                Capability::Authentication,
                Capability::Authentication.fixed_stages().to_vec(),
                vec![],
                vec![],
            ),
        ]);
        if configuration.get("quota_url").is_some() {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
                Capability::Quota,
                Capability::Quota.fixed_stages().to_vec(),
                vec![],
                vec![],
            )]);
        }
        let maintenance = configuration["maintenance"].as_bool().unwrap_or(false);
        if maintenance {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
        ...
        if configuration.get("request_profile_refresh").is_some() {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
        ...
        if configuration.get("account_operations").is_some() {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
                Capability::AccountManagement,
                Capability::AccountManagement.fixed_stages().to_vec(),
                vec![],
                vec![],
            )]);
        }
        if configuration.get("billing").is_some() {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
                Capability::Billing,
                vec![Stage::Execution],
        ...
        if configuration.get("model_discovery").is_some() {
            contributes.extend([super::contribution_for_id(
                &plugin_id,
                Capability::Models,
                Capability::Models.fixed_stages().to_vec(),
                vec![],
                vec![],
            )]);
        }
        ...
        let instance = PluginInstance {
            ...
            bindings: [
                ...
                maintenance.then(|| PluginCapabilityBinding {
                    contribution: format!("{plugin_id}.maintenance"),
    ...
    pub async fn runtime(&self) -> (Arc<PluginRuntime>, gateway_core::CoreBundle) {
        self.runtime_with_ports(self.store.provider_ports()).await
    }

    pub async fn runtime_with_ports(
        &self,
        ports: gateway_core::provider_ports::ProviderStorePorts,
    ) -> (Arc<PluginRuntime>, gateway_core::CoreBundle) {
        self.runtime_with_ports_and_drain(ports, ContinuationDrainConfig::default())
            .await
    }

    pub async fn runtime_with_continuation_drain(
        &self,
        drain: ContinuationDrainConfig,
    ) -> (Arc<PluginRuntime>, gateway_core::CoreBundle) {
        self.runtime_with_ports_and_drain(self.store.provider_ports(), drain)
            .await
    }

    async fn runtime_with_ports_and_drain(
        &self,
        ports: gateway_core::provider_ports::ProviderStorePorts,
        continuation_drain: ContinuationDrainConfig,
    ) -> (Arc<PluginRuntime>, gateway_core::CoreBundle) {
        let runtime = self.plugin_runtime(ports, continuation_drain);
        ...
        runtime
            .bind_model_ports(&nested_models, &affinity_lookup)
            .unwrap();
        (runtime, core)
    }
    ...
    pub async fn command_plane(
        ...
        let runtime = self.plugin_runtime(
            self.store.provider_ports(),
            ContinuationDrainConfig::default(),
        );
    ...
    fn plugin_runtime(
        ...
        Arc::new(
            PluginRuntime::new(
                ...
                gateway_core::engine::provider::ProviderRegistry::default(),
                ProviderAdminRegistry::new([]).unwrap(),
                Arc::new(gateway_host::outbound::HttpClient::new().unwrap()),
                Arc::new(gateway_host::process::ProcessSupervisor::new(
                    std::num::NonZeroUsize::new(128).unwrap(),
                )),
            )
            .with_provider_ports(ports)
            .with_network_policy(
    ...
    pub async fn bind_admin_accounts(
        ...
        gateway_admin::initialize_with_plugin_accounts(
            ...
            gateway_admin::AdminRuntimePorts {
                ...
                providers: runtime.admin_registry(core.snapshots()),
    ...
    pub async fn account(
        ...
        self.account_for("example", proxy).await
    }
    ...
    pub async fn administrator(&self, id: &str) {
        sqlx::query(sqlx::AssertSqlSafe(format!("insert into {}.admin_users(id, password_hash, created_at, updated_at) values ($1, 'test-only-disabled-password', now(), now())", self.schema)))
    ...
    pub async fn model_request_ledger(&self, client_key_id: &str) -> Vec<ModelRequestLedgerFact> {
        let rows: Vec<ModelRequestLedgerRow> = sqlx::query_as(sqlx::AssertSqlSafe(format!(
            "select model_request.id, model_request.provider_kind, model_request.request_kind,
                    model_request.subagent_kind, model_request.outcome,
                    model_request.completed_at is not null, model_request.cost_source,
                    model_request.cost_amount::text, charge.amount_usd::text
             from {}.model_requests model_request
             left join {}.client_key_charge_events charge
               on charge.request_id = model_request.id
             where model_request.client_api_key_ref = $1
             order by model_request.started_at, model_request.id",
...
pub fn mutation() -> MutationContext {
    MutationContext {
        ...
        request_id: "plugin-provider-test".into(),
    }
}
