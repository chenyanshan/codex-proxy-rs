...
use gateway_admin::{
    model::{
        ...
        provider_credentials::{
            PluginAccountListQuery, PluginAccountSaveReason, PreparedCredentialCreate,
            PreparedCredentialRotationFacts, PreparedPluginAccountSave, ProviderDocument,
        },
    },
...
impl PluginAccounts {
    ...
    pub(crate) async fn save_discovered_facts(
        &self,
        context: &CallContext,
        scope: &NetworkScope,
        account: &ProviderAccount,
        facts: CredentialFacts,
    ) -> Result<gateway_admin::model::provider_credentials::PluginAccountSaveResult, AdminError>
    {
        if !self.authorized {
            return Err(AdminError::new(AdminErrorKind::Forbidden, "账号访问未授权"));
        }
        if scope.account_id() != Some(account.id().as_str())
            || scope.credential_revision() != Some(account.revision().get())
        ...
        {
            return Err(AdminError::conflict("模型发现的账号事实已失效或未获授权"));
        }
        let authentication_kind = facts.authentication_kind.clone();
        let prepared = PreparedPluginAccountSave::Replace {
            facts: prepare_replace(
            ...
            .map_err(|_| AdminError::invalid("模型发现返回的账号事实不合法"))?,
            authentication_kind,
            reason: PluginAccountSaveReason::ModelDiscovery,
        };
        self.slot
            .upgrade()?
            .save(prepared, &mutation_context(context))
...
fn prepare_create(
    ...
    provider::validate_facts(&facts).map_err(|_| invalid())?;
    Ok(PreparedCredentialCreate {
        ...
        access_token_expires_at: provider::credential_timestamp(facts.access_token_expires_at_ms)
            .map_err(|_| invalid())?,
        next_refresh_at: provider::credential_timestamp(facts.next_refresh_at_ms)
            .map_err(|_| invalid())?,
        enabled: true,
...
fn prepare_replace(
    ...
    Ok(PreparedCredentialRotationFacts {
        ...
        replacement_identity: if preserve_identity {
            None
        } else {
            facts
                .upstream_user_id
