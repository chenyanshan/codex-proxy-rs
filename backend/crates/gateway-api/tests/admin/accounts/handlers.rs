...
#[tokio::test]
async fn manual_state_refresh_requires_admin_and_valid_account_id() {
    let fixture = AdminTestFixture::new().await;
    fixture.auth.insert_session("valid-session");
    for (body, authenticated, expected) in [
        ...
        (
            serde_json::json!({"accountId":"acct_test", "token":"must-not-be-accepted"}),
            true,
        ...
        let mut request = Request::builder()
            .method("POST")
            .uri("/api/admin/accounts/session-state/refresh")
            .header("x-request-id", "req_state_refresh")
        ...
        let response = admin::router::<AdminTestState>()
            ...
            .await
            .unwrap();
        assert_eq!(response.status(), expected, "request: {body}");
        assert_eq!(response.headers()[header::CACHE_CONTROL], "no-store");
    }
}
