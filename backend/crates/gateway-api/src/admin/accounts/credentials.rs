...
impl StartAccountAuthorizationRequest {
    pub fn validate(&self) -> Result<(), WireValidationError> {
        ...
        if serde_json::to_vec(&self.input).map_or(true, |data| data.len() > 64 * 1024) {
            return Err(WireValidationError::new("input"));
        }
    ...
    pub(super) fn into_command(
        ...
        Ok((
            ...
            StartAuthorization {
                context,
                input: ProviderDocument::new(OpaqueProviderData::new(self.input)),
                name: self.name,
                reauthorization,
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct PollAccountAuthorizationRequest {
    pub provider: String,
    pub flow_id: String,
...
impl PollAccountAuthorizationRequest {
    pub fn validate(&self) -> Result<(), WireValidationError> {
        validate_authorization_flow(&parse_provider(&self.provider)?, &self.flow_id)?;
        if let Some(callback) = &self.callback_url {
            require_text(callback, MAX_CALLBACK_URL_BYTES, "callbackUrl")?;
        }
        if let Some(settings) = &self.settings {
            settings.validate()?;
        }
    ...
    pub(super) fn into_command(
        self,
        context: gateway_admin::model::MutationContext,
    ) -> Result<(ProviderKind, PollAuthorization), WireValidationError> {
        self.validate()?;
        Ok((
            parse_provider(&self.provider)?,
            PollAuthorization {
...
pub enum AccountAuthorizationPollData {
    Pending { retry_after_ms: u64 },
    Complete { account_ids: Vec<String> },
}

impl From<AuthorizationPollResult> for AccountAuthorizationPollData {
    fn from(result: AuthorizationPollResult) -> Self {
        match result {
            AuthorizationPollResult::Pending { retry_after } => Self::Pending {
                retry_after_ms: retry_after.as_secs() * 1000
                    + u64::from(retry_after.subsec_millis()),
            },
            AuthorizationPollResult::Complete(result) => Self::Complete {
                account_ids: result
                    .accounts
                    .into_iter()
