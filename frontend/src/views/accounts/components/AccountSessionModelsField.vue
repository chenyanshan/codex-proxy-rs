...
function select(id: string, checked: boolean) {
  ...
  if (checked && selected.value.length >= 32) {
    inputError.value = '最多选择 32 个探活模型'
    return
  }
...
<template>
  <BaseFormItem label="State 探活模型" description="自动使用全局动态代理。每个模型独立发送 hi、提取并缓存 State，互不混用。请选择实际的上游模型 ID（最多 32 个）。">
    <div class="grid gap-3">
      <BaseScrollbar max-height="12rem">
        <div class="grid grid-cols-2 gap-2" role="group" aria-label="选择探活模型">
      ...
      <div class="flex gap-2">
        <BaseInput v-model="manual" class="min-w-0 flex-1" aria-label="自定义探活模型" placeholder="输入其他模型 ID" :disabled="disabled" @keydown.enter.prevent="add" />
