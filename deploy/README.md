...
部分客户端只使用 `config.toml` 里指定的模型，不显示可选模型列表。需要在客户端里切换模型时，
在管理端「账号管理」的账号行展开「更多操作 → 下载模型目录」，导出该账号的目录文件，
保存到本机任意可读路径，例如 `~/.codex/models.json`，再在 `config.toml` 中按绝对路径指定：

```toml
model_catalog_json = "/home/<user>/.codex/models.json"
```

目录只在客户端启动时加载，改完要完全退出并重启客户端。文件只含模型元数据，不含账号凭据。

导出时会请求该账号的上游原样目录，保留上游的模型标识、展示名称、推理强度、上下文窗口与服务档位。
配置了模型别名时，`model` 仍应填网关对外的模型标识；上游目录变化或账号换套餐后需要重新下载。
xAI 账号不提供这种原生目录，菜单只对 OpenAI 账号显示。
