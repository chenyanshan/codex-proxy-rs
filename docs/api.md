...
未收到上游 HTTP 响应的连接失败，在安全的请求内恢复耗尽后返回 `502`；上游超时返回 `504`。
没有可用路由、账号容量或本地建连资源等暂时不可服务的情况返回 `503`；调用方自身限流继续返回 `429`。
这些状态区分故障原因，不能保证任意客户端都会重试。已提交 SSE/WS 输出后不能更改 HTTP 状态，
只能通过既有流内错误合同结束。仅在存在可信等待时长时提供 `Retry-After`，不推测出口恢复时间。
发送前连接恢复的次数、时间和重放边界见 [核心执行不变量](architecture.md)。
...
超出时通常截断历史基线；唯一例外是原始前后观测确认的孤立时间异常：前后匹配同一窗口及套餐、
三点比例不下降且未跨越窗口边界时，只忽略异常读数，保留已记录用量。缺失邻居、连续异常或套餐改变仍截断；
不将宽时间容差当作上游窗口身份。历史文档缺少独立额度观测时间，
...
`GET /api/admin/car-quota?groupId=...` 返回周期状态、权重、`predictedCapacityUsd`（最新有效预测）、
...
成员用量使用已有 `weeklyUsedUsd` 字段承载所选周窗口内费用；账号周期模式由 `accountCycle` 标识。成员用量包含已撤销Key的历史费用，`id`为稳定成员标识、`revoked`表示凭据已撤销；凭据撤销不清账。
