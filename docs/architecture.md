...
Provider 在选定账号后应用代理位置覆盖。代理的自动位置与手动配置分别持久化，Store 将当前模式对应的位置投影到
账号合同；自动识别由 Admin 编排、Host 执行网络探测，只在开启、更换连接或管理员测试时触发，不注册后台刷新任务。
检测结果与配置原子写入，并以代理版本阻止旧结果覆盖新连接或新模式；生效位置变化沿用快照发布链路。
请求期间不查询 IP 归属或额外查询全局设置，配置发布不改变已开始请求的全局值。
...
car 的有效账号容量必须为正数，包含零 seat 状态；普通账号仍可不限并发。数据库约束保护账号独占、
seat 数量和单 seat 上限，配置事务失败时保持原配置。自助成员汇总只投影同 seat 的费用窗口，
撤销凭据不删除历史账本；Key 的逐请求查询仍按当前会话身份隔离。
...
OpenAI 已有会话的偏好账号仅受本地请求间隔阻塞时，开启账号排队后优先在现有预算内等待该账号，
以保留缓存连续性；队列关闭、满或超时后仍可选择可用备选账号，不把会话亲和升级为硬绑定。
...
管理提交重读持久化配置；扩展集合未变化时复用已发布的模型目录，先发布新的账号与权限事实，目录代次留待订阅任务或周期对账从 Provider 刷新，避免上游目录请求阻塞管理响应。
新提交会中断正在读取持久版本或编译目录的后台刷新，让提交先取得发布权；被中断的目录对账留待下个周期重试。
...
账号容量预测属于 Admin 的只读派生规则，不修改 quota 权威状态或金额结算。car 周期 Worker 可消费有效美元预测，
按独立的发布策略调整本地容量，但不能把预测当成上游真实余额。Store 通过专用采样端口在同一 SQL 快照内返回
截至观测时间的累计数值及有界历史 Provider 文档；原生 Provider 复用协议解析器解释文档，插件由 Runtime
本地解释 SDK 声明的版本化额度观测，不在历史查询中调用插件进程。观测通过 Core 原有请求结算持久化，
账号关联由实际执行上下文确定，窗口身份与归属必须匹配当前额度窗口。
每个采样点至多附带两个抽样前相邻文档；未解析或同完成时间的邻居不能证明连续性。Admin 仅在前后原始观测
匹配同一窗口、套餐与非下降比例时忽略孤立重置时间异常，不修改 Provider 权威快照；再按中立的额度事实
选择近期进度段预测剩余量，加本周期已记录用量形成周期总量。
